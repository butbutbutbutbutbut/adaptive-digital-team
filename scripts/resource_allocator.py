...
def _select_model(catalog: dict[str, Any], tier: str) -> dict[str, str] | None:
...
def _build_agent_config(catalog: dict[str, Any], tier: str) -> dict[str, Any] | None:
...
def allocate(
    ...
    tier = governance_plan.get("resource_tier") or LEGACY_TIER_MAP[risk]
    ...
    if recommended_points > hard_max_points:
        return _blocked(
            f"Point request {recommended_points} exceeds plan hard max {hard_max_points}."
        )
    ...
    maker = _build_agent_config(model_catalog, tier)
    if maker is None:
        return _blocked(
    ...
    if checker_now:
        checker = _build_agent_config(model_catalog, tier)
        if checker is None:
