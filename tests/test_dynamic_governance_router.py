...
18 tests (P1-T01 through P1-T18) plus edge cases.
...
# ═══════════════════════════════════════════════════════════
# P1-T11: Maker 与 Checker 不得为同一角色
# ═══════════════════════════════════════════════════════════

def test_p1_t11_maker_checker_separate():
    """Each step has exactly one executor_role. A step cannot be both MAKER and CHECKER."""
    plan = _plan({
        "request": "修复首页",
        "repository": "owner/project",
    })
    for step in plan["steps"]:
        # Each step has exactly one role
        role = step["executor_role"]
        assert role in ("HOLDER", "MAKER", "CHECKER", "HUMAN")
        # The plan-level checker_required means a separate checker audits
        # Individual steps are single-role
