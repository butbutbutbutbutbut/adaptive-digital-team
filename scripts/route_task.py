        ...
        if route == Route.FACT_SOURCE_REBIND:
            return [
                ExecutionStep(
                    ...
                    executor_role=ExecutorRole.HOLDER,
...
def _validate_plan_output(plan: GovernancePlan) -> None:
    ...
    for step in plan.steps:
        if _val(step.executor_role) not in {e.value for e in ExecutorRole}:
            raise ValueError(f"Illegal executor_role: {_val(step.executor_role)}")
